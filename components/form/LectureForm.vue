<template>
  <div>
    <slot>
      <h4 class="text-center text-3xl font-bold text-secondary-800">填寫表單諮詢課程優惠</h4>
      <p class="mb-4 mt-2 text-center text-lg text-base-400">
        請填寫表格內必要訊息，顧問專員將在收到訊息後儘速為您服務，謝謝。
      </p>
    </slot>

    <Form ref="form" v-slot="{ errors }" :validation-schema="$lectureSchema" @submit="onSubmit()">
      <div
      ...
      <div class="grid lg:grid-cols-3 lg:gap-4">
        <div>
          <div
            :class="errors.location ? 'border-red-500' : 'border-transparent'"
            class="flex flex-wrap items-center justify-center overflow-hidden rounded-sm border border-solid"
          >
            <label
              class="relative flex-[0_1_auto] bg-base-50 px-[8px] py-[12px] text-center font-bold after:absolute after:inset-y-0 after:right-0 after:m-auto after:h-[60%] after:w-[2px] after:bg-base-200 after:content-['']"
          ...
          <div class="h-4 w-full text-left text-sm text-red-500">{{ errors.location }}</div>
        </div>
        <div>
          <div
            :class="errors.session ? 'border-red-500' : 'border-transparent'"
            class="flex flex-wrap items-center justify-center overflow-hidden rounded-sm border border-solid"
          >
            <label
              class="relative flex-[0_1_auto] bg-base-50 px-[8px] py-[12px] text-center font-bold after:absolute after:inset-y-0 after:right-0 after:m-auto after:h-[60%] after:w-[2px] after:bg-base-200 after:content-['']"
            ...
            >
              ...
              <option value="第三場">第三場</option>
            </Field>
          </div>
          <div class="h-4 w-full text-left text-sm text-red-500">{{ errors.session }}</div>
        </div>
        <div>
          <div
            :class="errors.date ? 'border-red-500' : 'border-transparent'"
            class="flex flex-wrap items-center justify-center overflow-hidden rounded-sm border border-solid"
          >
            <label
              class="relative flex-[0_1_auto] bg-base-50 px-[8px] py-[12px] text-center font-bold after:absolute after:inset-y-0 after:right-0 after:m-auto after:h-[60%] after:w-[2px] after:bg-base-200 after:content-['']"
            ...
            >
              <option value="">請選擇</option>
              <option value="2023-05-01-19:00">2023-05-01-19:00</option>
            </Field>
          </div>
          <div class="h-4 w-full text-left text-sm text-red-500">{{ errors.date }}</div>
        </div>
      </div>
...
const { $lectureSchema } = useNuxtApp()
