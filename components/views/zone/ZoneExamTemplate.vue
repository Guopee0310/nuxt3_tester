        ...
        <ZoneTemplateXPadding class="-mt-10">
          ...
          <div v-show="isShow" class="bg-base-200 py-3">
            <div class="grid gap-2 px-2 text-center md:grid-cols-3 lg:grid-cols-6">
              <div
                v-for="item in comboTypeData"
                :key="item.comboTypeName"
        ...
        <ZoneTemplateXPaddingNarrow>
          <ZoneInfoContent>
            ...
            <ZoneInfoListLeft>
              ...
              <ZoneInfoArticleItem
                v-for="articleiItem in ArticlesData"
                :key="articleiItem.title"
              ...
              <div class="table-wrapper">
                <table class="w-full">
                  ...
                  <tbody class="text-center">
                    <template
                      v-for="comboExamItems in examComboDetailsData"
                      :key="comboExamItems.comboJobs"
                    >
                      <tr
                        v-for="(comboJobsItem, index) in comboExamItems.comboJobs"
                        :key="comboJobsItem.comboExamName"
                      >
                        <td
                          v-if="index == 0"
...
// 麵包屑設定
const breadcrumbInfo = ref(null) // 麵包屑設定
watchEffect(() => {
  examTypeId.value = route.query.exam_type_id
  examId.value = route.query.exam_data_id
...
useSeoMeta({
  title: () =>
    '高普考、初考、警察特考、鐵路特考、關務特考、司法特考、外交特考、調查局、國安局、地方特考、公職、公職考試、公務員/專區/公職考試',
  ogTitle: () => '考試資訊 - 公職考試',
  description:
    'TKB購課網擁有最專業的公職補習班-TKB百官網雲端課程，最詳細的公務人員考試資訊、公職考試科目、公職錄取率，包含高普考試、警察特考、鐵路特考、關務特考、司法特考、外交特考、調查局、國安局、地方特考等。',
  ogDescription:
  ...
  keywords:
    '高普考、初考、警察特考、鐵路特考、關務特考、司法特考、外交特考、調查局、國安局、地方特考、公職、公職考試、公務員'
